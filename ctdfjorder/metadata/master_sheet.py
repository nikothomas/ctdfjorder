...
from ctdfjorder.fjordphyto.generate_sites_db import generate_sites_database
...
class MasterSheet:
    ...
    def __init__(self, master_sheet_path: str,
        ...
        if ".csv" in path.basename(master_sheet_path):
            df = pl.io.csv.read_csv(
                master_sheet_path, columns=list_of_cols,
        ...
        self.data = self.data.with_columns(pl.col(latitude_label).cast(pl.Float64).alias(latitude_label),
                                           pl.col(longitude_label).cast(pl.Float64).alias(longitude_label))
        if self.with_crosschecked_site_names:
            sites_in_master_sheet = df.select(pl.col(site_names_label)).to_series().to_list()
    ...
    def find_match(
        ...
        closest_row_overall = self.data.select(
            pl.all().sort_by(
                (pl.col(self.datetime_utc_label) - timestamp_in_profile).abs()
            ).first()
        )
        ...
        secchi_depth = closest_row_overall.select(
            pl.col(self.secchi_depth_label)
        ...
        if not isinstance(latitude, float):
            raise_warning_improbable_match(f"Latitude: {latitude} is not float", filename=filename)
        if self.with_crosschecked_site_names:
            latitude_from_profile = profile.select(pl.col(LATITUDE_LABEL).first()).item()
            longitude_from_profile = profile.select(pl.col(LONGITUDE_LABEL).first()).item()
            if latitude_from_profile is not None and longitude_from_profile is not None:
                site_name_of_sample_master_sheet = closest_row_overall.select((pl.col(self.site_names_label))).item()
                for site in self.site_names_db.sites:
                    if site_name_of_sample_master_sheet is site.name:
                        if not abs(site.latitude - latitude_from_profile) < 0.2 and not abs(
                                site.longitude - longitude_from_profile) < 0.2:
                            message = (
                            ...
                            raise_warning_improbable_match(message=message, filename=filename)
        return Metadata(latitude=latitude, longitude=longitude, unique_id=unique_id, secchi_depth=secchi_depth)
